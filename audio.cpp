...
HRESULT InitAudio()
{
    ...
    ReverbConvertI3DL2ToNative( &g_PRESET_PARAMS[18], &native );
	...
	*/
	...
	g_audioState.wavInstanceAr = new CwavInstance[MAX_NUM_VOICES];
	for(int i=0;i<MAX_NUM_VOICES;i++)
		g_audioState.wavInstanceAr[i].initValues(details, ZMAX,
			g_audioState.nChannels, g_audioState.matrixCoefficients);
    ...
    g_audioState.emitter.DopplerScaler = 0.3f;
	*/
	/*
...
//-----------------------------------------------------------------------------
HRESULT PrepareAudio( const LPWSTR wavname, int index )
{
    if( !g_audioState.bInitialized )
        return E_FAIL;

	if(index>=MAX_NUM_VOICES||index<0)
		return E_FAIL;

	if(g_audioState.numVoices==0){
		if(index!=0)
			return E_FAIL;
		else
			g_audioState.numVoices++;
	}
	else{
		if(index==g_audioState.numVoices)
			g_audioState.numVoices++;
	}
	...
	*/
    ...
    wcscpy_s( wavFilePath, MAX_PATH, L"Wavs\\" );
	...
	V_RETURN(g_audioState.wavInstanceAr[index].prepareWav(cbWaveSize,hr,wav,
		g_audioState.pMasteringVoice, g_audioState.pSubmixVoice,
		g_audioState.pXAudio2, pwfx, index));
	/*
	...
	*/
	...
	//had to move this here, if start is called each loop the samples stack
	//and play through altogether at the start
	if(g_audioState.numVoices == MAX_NUM_VOICES){
		for(int i=0;i<3;i++){
			V_RETURN(g_audioState.wavInstanceAr[i].pSourceVoice->Start(0));
		}
		V_RETURN(g_audioState.wavInstanceAr[9].pSourceVoice->Start(0));
	}
	/*
	if(g_audioState.numVoices == MAX_NUM_VOICES){
		for(int i=0;i<MAX_NUM_VOICES;i++){
...
HRESULT UpdateAudio( float fElapsedTime )
{
    ...
    if( g_audioState.nFrameToApply3DAudio == 0 )
    {
        ...
        {
			for(int a=0;a<g_audioState.numVoices;a++){
				g_audioState.wavInstanceAr[a].emitter.InnerRadius = 2.0f;
				g_audioState.wavInstanceAr[a].emitter.InnerRadiusAngle = X3DAUDIO_PI/4.0f;
			}
        }
        else
        {
			for(int b=0;b<g_audioState.numVoices;b++){
				g_audioState.wavInstanceAr[b].emitter.InnerRadius = 0.0f;
				g_audioState.wavInstanceAr[b].emitter.InnerRadiusAngle = 0.0f;
			}
        }
        ...
        if( fElapsedTime > 0 )
        {
			...
			for(int c=0;c<g_audioState.numVoices;c++){
				D3DXVECTOR3 eVelocity = ( g_audioState.wavInstanceAr[c].vEmitterPos
					- g_audioState.wavInstanceAr[c].emitter.Position ) / fElapsedTime;
				g_audioState.wavInstanceAr[c].emitter.Position = g_audioState.wavInstanceAr[c].vEmitterPos;
				g_audioState.wavInstanceAr[c].emitter.Velocity = eVelocity;
			}
        }
		...
		for(int d=0;d<g_audioState.numVoices;d++){
			X3DAudioCalculate( g_audioState.x3DInstance, &g_audioState.listener,
				&g_audioState.wavInstanceAr[d].emitter, dwCalcFlags, &g_audioState.wavInstanceAr[d].dspSettings );

			IXAudio2SourceVoice* voice = g_audioState.wavInstanceAr[d].pSourceVoice;
			if( voice )
			{
				// Apply X3DAudio generated DSP settings to XAudio2
				voice->SetFrequencyRatio( g_audioState.wavInstanceAr[d].dspSettings.DopplerFactor );
				voice->SetOutputMatrix( g_audioState.pMasteringVoice, INPUTCHANNELS, g_audioState.nChannels,
										g_audioState.matrixCoefficients );

				voice->SetOutputMatrix(g_audioState.pSubmixVoice, 1, 1, &g_audioState.wavInstanceAr[d].dspSettings.ReverbLevel);

				XAUDIO2_FILTER_PARAMETERS FilterParametersDirect = { LowPassFilter, 2.0f * sinf(X3DAUDIO_PI/6.0f * g_audioState.wavInstanceAr[d].dspSettings.LPFDirectCoefficient), 1.0f }; // see XAudio2CutoffFrequencyToRadians() in XAudio2.h for more information on the formula used here
				voice->SetOutputFilterParameters(g_audioState.pMasteringVoice, &FilterParametersDirect);
				XAUDIO2_FILTER_PARAMETERS FilterParametersReverb = { LowPassFilter, 2.0f * sinf(X3DAUDIO_PI/6.0f * g_audioState.wavInstanceAr[d].dspSettings.LPFReverbCoefficient), 1.0f }; // see XAudio2CutoffFrequencyToRadians() in XAudio2.h for more information on the formula used here
...
VOID CleanupAudio()
{
    if( !g_audioState.bInitialized )
        return;
	//for(int ax=0;ax<MAX_NUM_VOICES;ax++){
		//delete g_audioState.wavInstanceAr[ax];
	
	for(int q=0;q<MAX_NUM_VOICES;q++)
		g_audioState.wavInstanceAr[q].clearVoice();
	...
	*/
	...
	delete[] g_audioState.wavInstanceAr;
	/*
    SAFE_DELETE_ARRAY( g_audioState.pbSampleData );
	*/
